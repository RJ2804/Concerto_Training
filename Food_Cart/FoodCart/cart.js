...
(function (global) {
  ...
  localStorage.removeItem("Mutton Biryani");
  localStorage.removeItem("Ambur Biryani");
  localStorage.removeItem("foodItems");
  localStorage.removeItem("Hyderabadi Biryani");

  function selectItemFromDropDown() {

    var itemValue = $('select#foodItems option:selected').val();

    prevSelectedCategory = itemValue;

    var invoiceDataWithMembership = foodItem.filter(item => item.category === itemValue)

    for (var items of invoiceDataWithMembership) {

      $('#foodcontainer').append("<div class="
        + items.category
        + "><div class="
        + '"row"'
        + ">"
  ...
  global.selectItemFromDropDown = selectItemFromDropDown;

})(window);

const selectElement = document.querySelector('#foodItems');

selectElement.addEventListener('change', (event) => {


  const boxes = Array.from(document.getElementsByClassName(prevSelectedCategory));
...
function clickedCheckBox(value, itemName) {

  var item = foodItem.filter(items => items.name === itemName)
  ...
  {
    otherFoodItems= localStorage.getItem('foodItem')
  }
...
{
localStorage.setItem("foodItem",(otherFoodItems+ ','+'{"name":"'+item[0].name+'"'+","+'"img":"'+item[0].img+'"'+ ","+'"price":"'+ item[0].price+'"'+","+'"quantity":"'+item[0].quantity+'"'+","+'"rating":"'+item[0].rating+'"'+"," +'"category":"'+item[0].category+'"'+"}"))

}
else
{
localStorage.setItem("foodItem",('{"name":"'+item[0].name+'"'+","+'"img":"'+item[0].img+'"'+ ","+'"price":"'+ item[0].price+'"'+","+'"quantity":"'+item[0].quantity+'"'+","+'"rating":"'+item[0].rating+'"'+"," +'"category":"'+item[0].category+'"'+"}"))
